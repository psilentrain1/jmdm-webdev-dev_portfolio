...
const db = new sqlite(path.join(process.cwd(), "folio.db"), {
  fileMustExist: true,
});

/* async function getUser(email: string): Promise<UserType | undefined> {
  try {
    const user = await db.prepare(`SELECT * FROM users WHERE email = ${email}`).all();
    return user[0];
  } catch (error) {
  ...
  providers: [
    Credentials({
      async authorize(credentials) {
        const parsedCredentials = z.object({ username: z.string().min(5), password: z.string().min(6) }).safeParse(credentials);
...
// import type { UserType, UserResponseType } from "@/types/user.types";
// import { AdapterUser } from "next-auth/adapters";
// import { CredentialsType, SocialCredentialsType } from "@/types/login.types";
...
//         try {
//           const user = await fetchUser(`api/auth/login`, {
...
//       authorize: async (credentials) => {
//         try {
//           const user = await fetchUser(`api/auth/social_login`, {
//             auth_code: typeof credentials.authCode === "string" ? credentials.authCode : "",
//           });
...
// async function fetchUser(url: string, body: CredentialsType | SocialCredentialsType) {
//   try {
